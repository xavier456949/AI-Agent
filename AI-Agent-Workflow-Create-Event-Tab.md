...
This workflow documents how to instruct the AI Studio to add new events into an existing tabbed Events page (e.g., a 2024 tab, 2025 tab, etc.). The user provides the event list directly — no image parsing required.

---

## Step 1 — Receive the Event List

The user provides the list of events to add as plain text, one event per line, in this format:
...
From this list, extract for each event:
- **Date** — parse from `DD MM月, YYYY` (e.g. `24 11月, 2017` → November 24, 2017)
- **Title** — everything after the second comma
- **Year tab** — derived from the YYYY year in the date (determines which tab the events belong to)

The user will also specify:
- Which **tab** on the clone site to add events to (e.g. `2017 tab`)
- Whether the tab is **empty** or already has events (and if so, which event to place the new ones after)

---

## Step 2 — Collect Event Details

For each event in the provided list, collect:

| Field | Where to Get It |
|---|---|
| **Date** | Parsed from the event list |
| **Title** | Parsed from the event list |
...
| **Source event page URL** | Find and click the matching event on the source site — this page's content will be cloned into the internal event page |
...
Check the event title text from the provided list:
...
3. **Number of events** — count the total events in the provided list.
...
```
Under [TAB NAME] tab, create [NUMBER] events [and place it after [ANCHOR EVENT TITLE]]. When click on the event, open within the site like events under 2026 tab and clone the content from the source page. For all images (feature images and all inline images inside the cloned content), download each image to local first, then upload from local to our site — do not hotlink from the original site. Process image downloads and uploads in batches of 5 at a time before moving to the next batch. [DATE 1] [EVENT TITLE 1], download feature image from [IMAGE_URL_1] to local then upload from local to our site, clone content from [SOURCE_EVENT_URL_1], download all inline images in the content to local then upload from local to our site, and use URL slug /event/[SLUG 1] [DATE 2] [EVENT TITLE 2], download feature image from [IMAGE_URL_2] to local then upload from local to our site, clone content from [SOURCE_EVENT_URL_2], download all inline images in the content to local then upload from local to our site, and use URL slug /event/[SLUG 2] [DATE 3] [EVENT TITLE 3], download feature image from [IMAGE_URL_3] to local then upload from local to our site, clone content from [SOURCE_EVENT_URL_3], download all inline images in the content to local then upload from local to our site, and use URL slug /event/[SLUG 3] [DATE 4] [EVENT TITLE 4], download feature image from [IMAGE_URL_4] to local then upload from local to our site, clone content from [SOURCE_EVENT_URL_4], download all inline images in the content to local then upload from local to our site, and use URL slug /event/[SLUG 4]
```

> **Critical:** Do not press Enter until the full prompt is complete. Pressing Enter submits immediately.

**Key points:**
- `download feature image from [IMAGE_URL] to local then upload from local to our site` — the two-step process (download → local → upload) ensures the image ends up hosted on the clone site (e.g. `vibe.filesafe.space/...`), not hotlinked from chinabaijiu.com.sg.
- `clone content from [SOURCE_EVENT_URL], download all inline images in the content to local then upload from local to our site` — every image in the event body goes through the same local download → upload process so all image URLs in the cloned content point to the clone site.
- `Process image downloads and uploads in batches of 5 at a time` — this instruction appears once at the top of the prompt and applies to all events. It prevents overloading the upload process.
...
All titles are Chinese → source: `https://chinabaijiu.com.sg/cn/%e6%b4%bb%e5%8a%a8/`
...
- **Exact titles:** Copy event titles exactly as provided in the user's event list.
...
- **Feature image = download to local, then upload from local:** Always download the feature image to local first, then upload from local to the clone site. This produces a hosted URL like `vibe.filesafe.space/...`. Never hotlink from chinabaijiu.com.sg — if their site goes down, hotlinked images will break.
- **Inline images = same local download → upload process:** Every image embedded in the cloned event body must also be downloaded to local first, then uploaded from local to the clone site. All image URLs in the cloned content should end up on the clone site, not the original.
- **Image upload batching:** Always include the instruction to process image downloads and uploads in batches of 5 at a time. This appears once at the top of the prompt and applies to the entire job.
...
- **One tab per prompt:** Do not mix events from different year tabs in one prompt.
