...
* Request the AI to generate a `sitemap_index.xml` that includes `page-index.xml`, `post-index.xml`, and `product-index.xml`.
...
* Ensure each page maintains the same URL slug as the original site.
* If a form is created, click "connect" to link it to the CRM system; this setup may take approximately 5 minutes.
* Keep all original internal and external links for every page.
